...
def get_download_dir():
    ...
    if DOWNLOAD_DIR is None:
        return os.path.expanduser("~/.cache/ses_edit/downloads/")
    return DOWNLOAD_DIR


def set_download_dir(dir):
    """
    Set a local download directory for storaging download files.
    """
    if dir is None:
        return
    if exists(dir) and get_local_rank() == 0:
        remove(dir)
    global DOWNLOAD_DIR
    DOWNLOAD_DIR = dir
    mkdir(DOWNLOAD_DIR)
...
def download_and_extract(path: str) -> str:
    ...
    path = download(path)
    cache_dir = os.path.expanduser("~/.cache/ses_edit/downloads/")
    ...
    if os.path.isfile(path):
        with tarfile.open(path) as tar:
            ...
            if get_local_rank() == 0:
                ...
                if not os.path.exists(os.path.join(cache_dir, folder_name)):
                    tar.extractall(cache_dir)
                if not os.path.exists(folder_name):
                    os.symlink(os.path.join(cache_dir, folder_name), folder_name)
                logger.info(
                    f"Creating link {folder_name} to" f" {os.path.join(cache_dir, folder_name)}."
                )
            # If distributed, all ranks must wait.
            barrier_if_distributed()
            path = folder_name
...
def listdir_with_metafile(path: str, overwrite: bool = True) -> List[str]:
    ...
    metafile_hdfs = os.path.join(path, "metafile.pkl")

    dirname = os.path.expanduser("~/.cache/ses_edit/downloads")
    os.makedirs(dirname, exist_ok=True)
    hashname = hashlib.md5(path.encode("utf-8")).hexdigest()
    filename = "metafile.pkl." + hashname
    metafile = os.path.join(dirname, filename)

    if exists(metafile) and (not overwrite):
        with open(download(metafile, overwrite=overwrite), "rb") as f:
            files = pickle.loads(f.read())
        return files

    # Write metafile only by global rank zero.
    if get_local_rank() == 0 and (overwrite or not exists(metafile_hdfs)):
        files = listdir(path)
        ...
        if get_global_rank() == 0:
            with tempfile.NamedTemporaryFile("wb", delete=True) as f:
                ...
                f.flush()
                copy(f.name, metafile_hdfs, blocking=True)
            logger.info(f"Created metafile for {path}")

    # All other ranks wait.
    barrier_if_distributed()

    if exists(metafile_hdfs):
        metafile = metafile_hdfs
