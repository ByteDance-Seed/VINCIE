...
def flatten(
    hid: List[torch.FloatTensor],  # List of (t, h, w, c)
) -> Tuple[
...
def get_seqlen(seq_shape: torch.LongTensor):
    return seq_shape.prod(-1)


def cu_seqlens(seq_len: torch.LongTensor, skip_empty: bool = True):
    if skip_empty and (seq_len == 0).any():
        seq_len = seq_len[seq_len > 0]
    return F.pad(seq_len.cumsum(0), (1, 0)).int()


def max_seqlen(seq_len: torch.LongTensor):
