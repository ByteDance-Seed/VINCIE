...
class NaDiT(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.cache = Cache(enable_precompute=kwargs.get("cache_precompute", False))
    ...
    def forward(
        self,
        vid: torch.FloatTensor = None,  # l c
        txt: Union[torch.FloatTensor, List[torch.FloatTensor]] = None,  # l c
        vid_shape: torch.LongTensor = None,  # b 3
        txt_shape: Union[torch.LongTensor, List[torch.LongTensor]] = None,  # b 1
        timestep: Union[int, float, torch.IntTensor, torch.FloatTensor] = None,  # b
        disable_cache: bool = False,  # for test
        is_forward_pie: bool = False, 
        prompt_id: torch.LongTensor = None, 
    ):
        if is_forward_pie:
            assert prompt_id is not None
            return self.prompt_id_embedding(prompt_id)

        cache = self.cache
        cache.reset(disable_cache)
        assert not cache.cache, f"{list(cache.cache.keys())=}"
        ...
        cache.mark_src(vid_shape)
        cache.mark_src(txt_shape)
        cache.precompute()
